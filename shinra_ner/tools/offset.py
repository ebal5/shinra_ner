...
def h2p(offset, len_list):
    ...
    _tag_len = sum(
        [elm for i, elm in zip(range(len(tl_slice)), tl_slice) if check(i)])
    pc = hc - _tag_len
    return (pr, pc)
...
def p2h(offset, len_list):
    ...
    idx = min([i for i in range(len(tsums)) if tsums[i] > pc], default=None)
    tag_len = sum([
        elm for i, elm in zip(range(len(tl[1])), tl[1])
        if not check(i) and i <= (2 * idx if tl[0] == Kind.TAG else 2 * idx -
                                  1)
    ])
    hc = pc + tag_len
    return (hr, hc)
